...
var d=getTop();
var e=dddIndex++;
d.loadJsFile("$js_path$webp/qmtip270d91.js",true);
d.waitFor(function(){
return d.QMTip;
},function(f){
if(f)
...
function isCanConnectGoogle()
{
if(isCanConnectGoogle.result===undefined)
{
var a=new Image();
a.style.cssText="display:none";
isCanConnectGoogle.result=false;
a.onload=function(){
isCanConnectGoogle.result=true;
};
a.src="//www.google.com/images/logo.png?r="+Math.random();
...
var i=getTop();
var e={'qq.com':1,'qqmail.com':1,'gtimg.cn':1,'gtimg.com':1,'soso.com':1};
var g=/^(?:[a-z]\w+?:)?\/\/([^\/]+)\//;
var f={};
function b(j)
{
...
a.src=QMDistributeDomain.addHost(b);
}
}
...
{
var c=getTop();
return !a&&c.gLogoUrl?c.gLogoUrl.replace(/(.*)_[^_]+_([^_]+)/,"$1_"+b+"_$2"):TE(['$images_path$webp/logo','$@$if($bFoxmail$)$@$','_foxmail','$@$else$@$','$sSubfolder$','$@$endif$@$','/logo_$nSkinId$_','$@$if($bFoxmail$)$@$','0','$@$else$@$','$sLogoid$','$@$endif$@$.gif']).replace({images_path:getPath("image"),bFoxmail:a,sSubfolder:c.gsLogoFolder,nSkinId:b,sLogoid:(c.gsLogoFolder||b==0)?(c.gLogoId||0):0});
}
...
setVerifyCallBack();
loadingBox({model:"\u9A8C\u8BC1\u7801",js:"$js_path$webp/qmverify27faad.js",oncheck:function(){
...
loadJsFileToTop(["$js_path$webp/com/kits/qmeditor/qqmail/release/editor29b6d5.js"]);
loadingBox({model:"\u53D1\u4FE1",js:["$js_path$webp/libcompose24e6ae.js","$js_path$webp/qmaddrinput24e6ae.js"],oncheck:function(){
...
if(S("tips",a))
{
S("tips",a).innerHTML=T(['<img src="$images_path$webp/ico_loading31e9c5d.gif" align=absmiddle>',' \u6B63\u5728\u6536\u53D6...&nbsp;\u7CFB\u7EDF\u5C06\u5728\u540E\u53F0\u81EA\u52A8\u6536\u53D6\uFF0C\u60A8\u53EF\u4EE5\u79BB\u5F00\u6B64\u9875\u9762\uFF0C\u7A0D\u540E\u56DE\u6765\u67E5\u770B\u6536\u53D6\u7ED3\u679C\u3002']).replace({images_path:getPath("image",true)});
}
recvPopHidden(b);
}
...
var j=getEventTarget(d),f;
if(attr(j,"t")=="5")
{
c.QMReadMail.addCalEvent(j,d,b);
f=true;
}
else if(attr(j,"t")=="7")
{
...
try{
var l=getTop(),k=l.calcPos(j),h=l.S("showcalpanel",l.getMainWin()),i=l.calcPos(h),g=l.calcPos(l.S("contentDiv",l.getMainWin()));
if(i[2]>g[2])
{
h.style.top=k[2]-i[2]+"px";
}
}
catch(m)
{
...
else if(a.className=="qqmail_card_reply"||a.className=="qqmail_card_reply_btn"||a.className=="qqmail_birthcard_reply"||a.className=="qqmail_birthcard_reply_btn")
{
var n=a.name,l=a.className,f=!!n,d=l.indexOf("birthcard")!=-1;
getMainWin().location=T('/cgi-bin/cardlist?sid=$sid$&t=$t$&s=$s$&today_tips=$tips$&loc=readmail,readmail,sendnewcard,1&ListType=$listtype$&email=$email$$newwin$').replace({sid:getSid(),t:f?"compose_card":"card",s:d?"replybirthcard":"",tips:l.indexOf("btn")!=-1?"112":"111",listtype:f?"No":"Cards&Cate1Idx=listall",email:n,newwin:getTop().bnewwin?"&newwin=true":""});
return false;
}
...
if(j)
{
getMainWin().location=T("/cgi-bin/readmail?sid=$sid$&mailid=$mailid$&t=compose&s=reply&disptype=html").replace({sid:getSid(),mailid:j.getMailId()});
}
return false;
}
...
{
var n=a.name;
openComposeDlg("card",{sTitle:"\u7B54\u8C22\u597D\u53CB",sDefAddrs:n,bAddrEdit:true,nWidth:495,sDefContent:"\u8C22\u8C22\u4F60\u7684\u8D3A\u5361\uFF01 \u4EE5\u540E\u8981\u5E38\u8054\u7CFB\u54E6\u3002",bContentEdit:true,sDefSubject:"\u8C22\u8C22\u4F60\u7684\u8D3A\u5361!",bRichEditor:false,oncomplete:function(){
},bShowResult:true});
return false;
}
...
{
var n=a.name;
openComposeDlg("postcard",{sTitle:"\u7B54\u8C22\u597D\u53CB",sDefAddrs:n,bAddrEdit:true,nWidth:495,sDefContent:"\u8C22\u8C22\u4F60\u7684\u660E\u4FE1\u7247\uFF01 \u4EE5\u540E\u8981\u5E38\u8054\u7CFB\u54E6\u3002",bContentEdit:true,sDefSubject:"\u8C22\u8C22\u4F60\u7684\u660E\u4FE1\u7247!",bRichEditor:false,oncomplete:function(){
},bShowResult:true});
return false;
}
...
{
var q='',p='',j=k.QMReadMail;
if(j)
{
try{
var h=(j.getSubMailWithDom?j.getSubMailWithDom(a):j.getMailInfo()).from;
q=h&&h.name||'';
p=h&&h.addr||'';
}
catch(u)
...
goUrlMainFrm(T('/cgi-bin/readtemplate?sid=$sid$&t=compose_postcard&email=$email$&reply=1&frname=$name$&fraddr=$addr$').replace({name:escape(q),addr:escape(p),sid:getSid(),email:a.name}),false);
return false;
}
...
if(j)
{
window.open(T('/cgi-bin/readmail?sid=$sid$&t=print_haagendazs&s=print&filterflag=true&mailid=$mailid$').replace({sid:getSid(),mailid:j.getMailId()}));
}
return false;
}
...
if(j)
{
window.open(T('/cgi-bin/today?t=haagendazs2010&sid=$sid$').replace({sid:getSid(),mailid:j.getMailId()}));
}
return false;
}
...
{
var m=a.getAttribute("ctype");
var s=a.getAttribute("param_new");
var t="";
if(s.indexOf("follow=1")>=0)
{
var o=a.getAttribute("followuin");
t=(getTop().gsRssDomain||"")+"/cgi-bin/reader_mgr";
QMAjax.send(t,{method:"POST",content:"fun=followshare&followuin="+o+"&sid="+getSid(),onload:function(e,v){
if(e)
{
getMainWin().location=T('$host$/cgi-bin/reader_article_list?sid=$sid$&$param$').replace({host:(getTop().gsRssDomain||""),sid:getSid(),param:s});
}
}});
}
else{
...
if(m=="onefeed")
{
t=(getTop().gsRssDomain||"")+"/cgi-bin/reader_mgr?fun=setlog&flag=3&from=2";
}
else{
t=(getTop().gsRssDomain||"")+"/cgi-bin/reader_mgr?fun=setlog&flag=3&from=4";
}
runUrlWithSid(t);
return false;
}
...
var t=(getTop().gsRssDomain||"")+"/cgi-bin/reader_mgr?fun=setlog&flag=3&from=3";
runUrlWithSid(t);
return false;
}
...
getMainWin().location=T('$host$/cgi-bin/reader_detail?sid=$sid$&$param$').replace({host:(getTop().gsRssDomain||""),sid:getSid(),param:i.join("&")});
return false;
}
...
getMainWin().location=T('$host$/cgi-bin/reader_list?classtype=allfriend&refresh=1&share=1&sid=$sid$&$param$').replace({host:(getTop().gsRssDomain||""),sid:getSid(),param:i.join("&")});
return false;
}
...
getMainWin().location=T('$host$/cgi-bin/reader_catalog_list?sid=$sid$&classtype=share&share=1&refresh=1&$param$').replace({host:(getTop().gsRssDomain||""),sid:getSid(),param:i.join("&")});
return false;
}
...
var r=getMainWin().location.href;
if(b!="preview"&&r.indexOf('/cgi-bin/readmail?')==-1&&r.indexOf('t=mail_list_ad')==-1&&r.indexOf('t=unreadmail')==-1&&r.indexOf('t=r_index')==-1)
{
return true;
}
var g=a.parentNode;
while(g)
{
if(g.nodeType==1&&(g.id=="QQmailNormalAtt"||g.id=="attachment"))
{
return true;
}
g=g.parentNode;
}
window.open(T('$baseDomain$/cgi-bin/mail_spam?action=check_link&url=$url$&mailid=$mid$&spam=$spam$').replace({baseDomain:getTop().getTopHost(),mid:c||getTop().getUrlParams(getMainWin().location)['mailid'],spam:b=="spam"?1:0,url:encodeURI(a.href)}),"_blank");
return false;
}
...
{
loadJsFile("$js_path$webp/qmtip270d91.js",true);
...
{
QMAjax.send(T("/cgi-bin/mail_mgr?sid=$sid$&fname=&mailaction=mail_filter").replace({sid:getSid()}),{method:"GET",headers:{"If-Modified-Since":"0","Cache-Control":"no-cache, max-age=0"},onload:function(b,c){
if(b)
{
...
{
cacheByIframe(a&&a.defcss==false?[]:[["css","",getRes("$css_path$webp/comm2762b5.css")],["css",getPath("style"),"skin"]],extend({className:"menu_base_if",transparent:false,destroy:false},a,{win:b,header:["<script>",getTop.toString().replace(/[\r\n]/g,""),"<\/script>",a&&a.header||""].join(""),onload:function(c){
...
{
var b=new QMDialog({sId:"LoAdINgBOx",sTitle:a.model+"\u6A21\u5757\u52A0\u8F7D\u4E2D...",nWidth:400,nHeight:187,sBodyHtml:T(['<div style="text-align:center;padding:58px; padding-bottom:78px;">','<img id="load" src="$images_path$webp/ico_loading21e9c5d.gif">','<span id="err" style="display:none;">$model$\u6A21\u5757\u52A0\u8F7D\u5931\u8D25</span>','</div>']).replace(extend(a,{images_path:getPath("image")})),onclose:function(){
b=null;
}});
if(a.js)
{
...
var k=function(l){
var n=getTop(),o=n.T("/cgi-bin/loginpage?t=loginpage_relate&account=$account$&relateuin=$uin$").replace({uin:i,account:h.email}),m=new n.QMDialog({sId:"RelataPtlogin",sTitle:"\u5BC6\u7801\u9A8C\u8BC1",nWidth:440,nMinWidth:440,nHeight:236,sUrl:o,onshow:function(){
n.S(["RelataPtlogin","QMDialog"].join("_"),getTop()).style.minWidth=0;
}});
n.fAfterLogin=function(){
QMAjax.send(["/cgi-bin/uplinktime?t=login_relate_after&sid=",getSid(),"&uin=",i].join(""),{onload:function(p,q){
if(p)
{
var r=evalValue(["(",q,")"].join(""));
if(r.result=="0")
{
showInfo("\u9A8C\u8BC1\u6210\u529F");
l&&l();
delete n.fAfterLogin;
delete n.fLoginResize;
}
else{
showError("\u9A8C\u8BC1\u5931\u8D25");
}
...
if(p>0)
{
m.option("nHeight",p+36);
}
m.S("_dlgiframe_").setAttribute("height",p);
};
},j=function(){
...
var c=getTop();
var d=["url="+encodeURIComponent("https://api.linkedin.com/v1/people/~"+":(id,formatted-name,picture-url,email-address,main-address,headline,industry,location,public-profile-url,positions:(is-current,title,company))?format=json"),"type=GET","action=proxy","header="+encodeURIComponent("Host: api.linkedin.com"),"header="+encodeURIComponent("Connection: keep-alive"),"header="+encodeURIComponent("Authorization: Bearer "+b)].join("&");
...
var e=c.finds('.contentcontainer',S("securityTiprlt"))[0];
c.addEvent(e,'mouseleave',function(f){
c.stopPropagation(f);
setTimeout(function(){
...
loadJsFile("$js_path$webp/qmlinkman24e6ae.js",true,e,b);
loadJsFile("$js_path$webp/qmaddress24e6ae.js",true,e,b);
}
...
if(!b.QMPlayer)
{
loadJsFileToTop(["$js_path$webp/qmplayer/player289cd9.js"]);
}
waitFor(function(){
return !!b.QMPlayer;
},function(c){
...
if(!c.QMPlayer)
{
loadJsFile("$js_path$webp/qmplayer24e6ae.js",true,c.document);
}
return b(a);
}
function playUrl(a)
...
{
var b=getTop();
b.loadCssFile("$css_path$webp/webpushtip2762b5.css",true);
b.loadJsFile("$js_path$webp/qmwebpushtip251e7c.js",true,b.document,function(){
b.QMWebpushTip.open(a);
});
b.loadJsFile("$js_path$webp/qmwebpush24e6ae.js",true,b.document);
}
...
c&&showInfo("\u52A0\u8F7D\u4E2D\uFF0C\u8BF7\u7A0D\u5019...");
getTop().loadJsFile("$js_path$webp/gift/app24e6b9.js",true,getTop().document,function(){
hiddenMsg();
if(b.indexOf("msg:")==0)
{
...
var f=c&&c[l]&&c[l][0];
try{
var j=f.loc,d=f.oid,h=f.fodder,m={loc:j,oid:d,hit:0},g=T("stat=log_ad_show&loc=$loc$&oid=$oid$&hit=$hit$&resource_url=$resource_url$&link_to=$link_to$&loc=ad_today,$loc$,$oid$,$hit$");
if(h[0]&&j&&!(d=="1"||d=="100"))
{
b._oADData[j]=extend(h[0],{cid:f.cid,loc:f.loc,oid:f.oid,monitor_url:f.monitor_url});
m=extend(m,{hit:1,link_to:encodeURIComponent(h[0].link_to),resource_url:encodeURIComponent(h[0].resource_url)});
ossLog("delay","all","kw="+j);
}
b._postOssLog(m);
}
catch(k)
...
var n=b._getADDomByloc(a,e),i=attr(n,"callback");
if(b._oADData[e])
{
b._defAction(a,b._oADData[j],j);
}
else{
...
try{
var m=this,l=this._getADDomByloc(c,d),n=d+"_showed",h=location.protocol=="https:",o=h?T("https://stockweb.mail.qq.com/p?oid=$oid$&cid=$cid$&loc=$loc$&pdomain=p.l.qq.com"):T("http://p.l.qq.com/p?oid=$oid$&cid=$cid$&loc=$loc$"),g=attr(l,"bgimg")=="1"?true:false,f=attr(l,"closebtn")=="1"?true:false,i=attr(l,"pos");
if(l&&b.resource_url)
{
var k=l.parentNode;
h&&(b.resource_url=this._getHttpsUrl(b.resource_url)),_sADHtml=this._getADTmpl(g?1:2),_oTodayBar=S("todaybarTitle",c);
if(_oTodayBar)
{
show(_oTodayBar,1);
...
{
var b=getTop(),a=arguments.callee;
b.loadCssFile("$css_path$webp/login/addAccount2762b5.css",true,b.document);
...
_oTop.loadJsFile("$js_path$webp/safeauth24e6ae.js",true);
...
},doGoogleOauth:function(a,D){
var v=getTop(),q=this,B=a._sPopmail,C=a._sPoppwd,o=q._oDialog.S("ipt_pwd"),e=q._oDialog.S("geConfirmBtn");
if(v.attr(e,"oauth")=="true")
{
return;
}
v.attr(e,"oauth","true");
v.addClass(e,"qui_btn_BlueDisabled");
var i=q._oDialog.S("div_verifycode"),w=q._oDialog.S("verifyImage"),l=q._oDialog.S("loginData"),k=q._oDialog.S("loginCookies"),m=q._oDialog.S("loginVerify");
var h=q._oDialog.S("div_secondFactor"),t=q._oDialog.S("smsData"),s=q._oDialog.S("smsCookies"),r=q._oDialog.S("smsAction"),u=q._oDialog.S("smsUserPin");
var g=q._oDialog.S("div_challenge"),d=q._oDialog.S("challengeData"),c=q._oDialog.S("challengeCookies"),n=q._oDialog.S("phoneNumber"),x=q._oDialog.S("verifySmsChallengeAnswer"),j=q._oDialog.S("emailAnswer"),f=q._oDialog.S("deviceAddress"),p=q._oDialog.S("secretAnswer"),b=q._oDialog.S("challengeTip");
var H=function(I,J){
...
x.style.display="";
x.value="";
b.innerHTML="(\u8BF7\u8F93\u5165\u4F60\u7684Gmail\u5E10\u53F7\u7ED1\u5B9A\u7684\u624B\u673A\u53F7\u7801\u6536\u5230\u7684\u77ED\u4FE1\u9A8C\u8BC1\u7801)";
x.focus();
}
...
n.style.display="";
n.value="";
b.innerHTML="(\u8BF7\u8F93\u5165\u4F60\u7684Gmail\u5E10\u53F7\u7ED1\u5B9A\u7684\u624B\u673A\u53F7\u7801\u6765\u5B8C\u6210\u8EAB\u4EFD\u9A8C\u8BC1)";
n.focus();
}
...
f.style.display="";
f.value="";
b.innerHTML="(\u8BF7\u8F93\u5165\u4F60\u7684Gmail\u5E10\u53F7\u5E38\u767B\u5F55\u7684\u4F4D\u7F6E\u6765\u5B8C\u6210\u8EAB\u4EFD\u9A8C\u8BC1)";
f.focus();
}
...
p.style.display="";
p.value="";
b.innerHTML='(\u8BF7\u56DE\u7B54\u5BC6\u4FDD\u95EE\u9898 \u201C<span style="color:red;">'+J+'</span>\u201D \u6765\u5B8C\u6210\u8EAB\u4EFD\u9A8C\u8BC1)';
p.focus();
}
...
j.style.display="";
j.value="";
b.innerHTML="(\u8BF7\u8F93\u5165\u4F60\u7684Gmail\u5E10\u53F7\u7ED1\u5B9A\u7684\u8F85\u52A9\u90AE\u7BB1\u5730\u5740\u6765\u5B8C\u6210\u8EAB\u4EFD\u9A8C\u8BC1)";
j.focus();
}
else{
v.LogKV("getinvestigate|googleoauth|fail|challengetype");
q.doRealAction(a);
}
};
q.hideWarn();
...
{
var G=["smsAction="+r.value,"smsUserPin="+u.value,"smsData="+t.value,"smsCookies="+s.value].join("&");
if(i.style.display!="none")
{
G=[G,"strongauthcaptcha="+m.value].join("&");
}
v.QMAjax.send("/cgi-bin/googleoauth/secondFactor",{content:G,method:"POST",onload:function(I,J){
var K=v.evalValue(J);
if(I)
{
if(!K.errCode&&K["access_token"])
{
v.LogKV("getinvestigate|googleoauth|oauth|success");
D&&D(K);
}
else if(K.errCode=="104")
{
v.attr(e,"oauth","false");
v.rmClass(e,"qui_btn_BlueDisabled");
if(K.verifyImageUri)
{
w.setAttribute("src",decodeURIComponent(K.verifyImageUri)+"&sid="+v.getSid());
m.value="";
i.style.display="";
}
else{
...
r.value=K.smsAction;
t.value=K.smsData;
s.value=K.smsCookies;
q.showWarn("\u8EAB\u4EFD\u9A8C\u8BC1\u5668\u9A8C\u8BC1\u5931\u8D25\uFF0C\u8BF7\u91CD\u65B0\u8F93\u5165");
}
else{
v.LogKV("getinvestigate|googleoauth|oauth|fail");
i.style.display="none";
...
{
var G=["Email="+B,"&Passwd="+C,"&logincaptcha="+m.value,"&loginData="+l.value,"&loginCookies="+k.value].join("");
v.QMAjax.send("/cgi-bin/googleoauth/verifyLogin",{content:G,method:"POST",onload:function(I,J){
var K=v.evalValue(J);
if(I)
{
if(!K.errCode&&K["access_token"])
...
i.style.display="none";
h.style.display="none";
d.value=K.challengeData;
c.value=K.challengeCookies;
g.style.display="";
H(K.challengeType,K.secretQuestion);
}
else{
v.attr(e,"oauth","false");
v.rmClass(e,"qui_btn_BlueDisabled");
...
{
y=[d.value,"challengetype%3DVerifySmsChallenge","VerifySmsChallengeAnswer%3D"+x.value].join("%26");
z="VerifySmsChallenge";
}
else if(n.style.display!="none")
{
y=[d.value,"challengetype%3DPhoneVerificationChallenge","phoneNumber%3D"+n.value].join("%26");
z="PhoneVerificationChallenge";
}
else if(f.style.display!="none")
{
y=[d.value,"challengetype%3DUnverifiedPhoneChallenge","deviceAddress%3D"+f.value].join("%26");
z="UnverifiedPhoneChallenge";
}
else if(p.style.display!="none")
{
y=[d.value,"challengetype%3DSecretQuestionChallenge","answer%3D"+p.value].join("%26");
z="SecretQuestionChallenge";
}
else{
y=[d.value,"challengetype%3DRecoveryEmailChallenge","emailAnswer%3D"+j.value].join("%26");
z="RecoveryEmailChallenge";
}
var G=["challengeData="+y,"&challengeCookies="+c.value].join("");
v.QMAjax.send("/cgi-bin/googleoauth/challenge",{content:G,method:"POST",onload:function(I,J){
var K=v.evalValue(J);
if(I)
{
if(!K.errCode&&K["access_token"])
...
v.attr(e,"oauth","false");
v.rmClass(e,"qui_btn_BlueDisabled");
v.LogKV("getinvestigate|googleoauth|oauth|challengefail");
d.value=K.challengeData;
c.value=K.challengeCookies;
q.showWarn("\u8EAB\u4EFD\u9A8C\u8BC1\u5931\u8D25\uFF0C\u8BF7\u91CD\u65B0\u9A8C\u8BC1");
if(z=="VerifySmsChallenge")
{
x.value="";
x.focus();
}
else if(z=="PhoneVerificationChallenge")
{
n.value="";
n.focus();
}
else if(z=="UnverifiedPhoneChallenge")
{
f.value="";
f.focus();
}
else if(z=="SecretQuestionChallenge")
{
p.value="";
p.focus();
}
...
var G="Email="+B+"&Passwd="+C;
v.QMAjax.send("/cgi-bin/googleoauth",{content:G,method:"POST",onload:function(I,J){
var K=v.evalValue(J);
if(I)
{
if(!K.errCode&&K["access_token"])
...
b.loadCssFile("$css_path$webp/login/bindAccount24e6b9.css",true,b.document);
...
if(!this._msVer)
{
var a=getTop().getRes("$css_path$webp/qui_dialogPtlogin2762b5.css"),b=(/qui_dialogPtlogin(.*)\.css/.test(a)&&RegExp.$1)||"";
this._msVer=b;
}
return this._msVer;
},getDefaultCfg:function(){
...
var a={'comm2010.css':'$css_path$webp/comm201029616a.css'};
return function(b){
var c=a[b];
return c&&getRes(c);
};
